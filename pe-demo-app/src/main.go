...
func main() {
  // test for ENV environment variable
  devOrProd := os.Getenv("ENV")
  if devOrProd == "" {
    log.Printf("Environment variable 'ENV' not set!\n")
  }
...
func hello(w http.ResponseWriter, r *http.Request) {
  ...
  if os.Getenv("ENV") == "" {
    w.WriteHeader(http.StatusServiceUnavailable)
    w.Write([]byte("503 - Error due to missing environment variable!\n"))
    return
  }
  ...
  fmt.Fprintf(w, "I have been hit [%v] times since deployment!\n", count)
}
